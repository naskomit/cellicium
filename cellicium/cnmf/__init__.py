...
import scanpy as sc
import cellicium.multiproc as cmproc
from cellicium.logging import logging as log
...
class CNMFRunner(object):
    def __init__(self, name, work_dir, num_workers = 1):
        self.name = name
        self.work_dir = work_dir
        self.num_workers = num_workers
        self._current_consensus = None

    def get_cnmf(self):
        cnmf_obj = cNMF(output_dir = self.work_dir, name = self.name)
        cnmf_obj._initialize_dirs()
        return cnmf_obj
    ...
    def prepare(self, adata, num_highly_variable_genes = 2000, num_components = 10, num_iterations = 20, random_seed = 42, beta_loss = 'frobenius', clean_dir = True):
        ...
        norm_counts = cnmf_obj.get_norm_counts(input_counts, tpm, num_highvar_genes = num_highly_variable_genes)
        if norm_counts.X.dtype != np.float64:
            norm_counts.X = norm_counts.X.astype(np.float64)
    ...
    def set_current_consensus(self, num_components : int, density_threshold : float = 0.5, local_neighborhood_size : float = 0.3):
        self._current_consensus = {
    ...
    def get_current_consensus(self):
        if self._current_consensus is None:
            raise ValueError('No current consensus. Either run `consensus` method or use `set_current_consensus`')
        else:
            return self._current_consensus

    def consensus(self, density_threshold = 0.5, local_neighborhood_size = 0.3, show_clustering = True):
        cnmf_obj = self.get_cnmf()
        run_params = load_df_from_npz(cnmf_obj.paths['nmf_replicate_parameters'])
        ks = sorted(set(run_params.n_components))
        for k in ks:
    ...
    def show_consensus_matrix(self):
        ...
        filename = f'{self.name}.clustering.k_{consensus["num_components"]}.dt_{density_threshold_str}.png'
    ...
    def top_genes(self, adata, n = 50):
        genes = []
        for gep in adata.uns['programs']:
            gene_scores = adata.var[gep].sort_values(ascending = False)
            genes.append(gene_scores.index.values[:n])

        genes = pd.DataFrame(genes, index = adata.uns['programs']).T
        return genes

    def top_gene_scores(self, adata, n = 50):
        gene_scores = []
        for gep in adata.uns['programs']:
            scores = adata.var[gep].sort_values(ascending = False)
            gene_scores.append(scores.values[:n])

        gene_scores = pd.DataFrame(gene_scores, index = adata.uns['programs']).T
        return gene_scores